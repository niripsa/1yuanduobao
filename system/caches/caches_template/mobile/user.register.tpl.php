...
<script type="text/javascript" src="<?php echo G_TEMPLATES_JS; ?>/jquery.Validform.min.js?ver=1.1"></script>
<?php echo js("jquery.pageDialog"); ?>
<?php include self::includes("index.header_top"); ?>

<section>
    <form action="<?php echo WEB_PATH; ?>/member/user/MobileRegister/<?php echo $decode; ?>" method="post" id="registerform" name="registerform">
    <div class="registerCon">
        <ul>
            <li><input id="usr" datatype="m | e" sucmsg="帐号验证通过！" name="user" type="tel" placeholder="请输入您的手机号码" class="rText"><s class="rs1"></s>
            </li>
            <li><input id="pwd" type="password" datatype="*6-20" plugin="passwordStrength"  name="pass" placeholder="请填写长度为6到20的密码" class="rText"><s class="rs3"></s>
            </li>
            <li><span id="msg"></span></li>
            <li><a id="btnNext" class="nextBtn orgBtn">下一步</a></li>            
            <li><span id="isCheck"><em></em>我已阅读并同意</span> <a href="javascript:xieyi()"><?php echo _cfg("web_name"); ?>用户服务协议</a></li>
        </ul>
    </div>
    </form>
</section>
<section style="display: none">
 <h2>服务协议</h2>
        ...
        <p>7、90天未达到“总需参与人次”的商品，用户可通过客服申请退款，所退款项将在3个工作日内，退还至一元夺宝账户中。</p>
        <h3>六、配送及费用</h3>
    ...
    $(function(){
        var demo = $("#registerform").Validform({       
                tiptype:function(msg,o,cssctl){
                    var objtip=o.obj.parent().next().find(".rText");
                ...
                passwordstrength:{
                    ...
                    trigger:function(obj,error){
                        if(error){
                            obj.parent().next().find(".rText").show();
                            obj.parent().next().find(".passwordStrength").hide();
                        }else{                      
                            obj.parent().next().find(".rText").hide();
