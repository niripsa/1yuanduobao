<?php defined('G_IN_SYSTEM')or exit('No permission resources.'); ?><?php echo css("style"); ?>
<?php echo css("demo"); ?>
...
<div class="login_layout">
    <div class="login_title">
        ...
        <span>已经是会员？<a id="hylinkLoginPage" class="blue Fb" href="<?php echo WEB_PATH; ?>/login">登录</a></span>
    </div>
    <div class="login_ConInput registerform">
        <form method="post" id="registerform" action="<?php echo WEB_PATH; ?>/member/user/UserRegister/<?php echo $decode; ?>" enctype="application/x-www-form-urlencoded">
        <dl>
            <dt>邮箱地址或手机号码：</dt>
            <dd><input id="ipyanz" datatype="m | e" sucmsg="帐号验证通过！" nullmsg="请填写帐号！" errormsg="手机号或邮箱！" type="text"  name="user" class="inputxt" /></dd>
            <dd><div style="margin:7px 0 0 10px;" class="Validform_checktip">手机号或邮箱！</div></dd>
        </dl>
        <dl>
            <dt>密码：</dt>
            <dd><input datatype="*6-20" plugin="passwordStrength" nullmsg="请设置密码！" errormsg="密码范围在6~20位之间！" type="password" name="pass" class="inputxt" maxlength="20"/></dd>
            <dd>
        ...
        <dl>
            <dt>确认密码：</dt>
            <dd><input tip="test"  datatype="*" ignore="ignore" recheck="pass" nullmsg="请再输入一次密码！" errormsg="您两次输入的账号密码不一致！" type="password" name="pass2" class="inputxt" maxlength="20"/></dd>
            <dd><div style="margin:7px 0 0 10px;" class="Validform_checktip"></div></dd>
        </dl>
          <dl>
                <dt>验证码：</dt>
                <dd><input class="inputxt" name="code" type="text" ajaxurl="<?php echo G_WEB_PATH; ?>/?plugin=1&api=Captcha&action=check"  datatype="*4-6" nullmsg="请输入验证码！" errormsg="请输入正确的验证码！"/>
                    <span class="fog"><img id="checkcode" src="<?php echo G_WEB_PATH; ?>/?plugin=true&api=Captcha"/></span></dd>
                <dd><div style="margin:7px 0 0 10px;" class="Validform_checktip"></div></dd>
            </dl>
...
<script type="text/javascript">
    ...
    <div class="Service_Agreement">    
        ...
        <p>7、90天未达到“总需参与人次”的商品，用户可通过客服申请退款，所退款项将在3个工作日内，退还至夺宝账户中。</p>
        <h3>六、配送及费用</h3>
