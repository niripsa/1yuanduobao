...
class Captcha_plugin {
    ...
    /*生成点*/
    public function Img_To_Dian($num=10,$color=NULL){
        $this->Createimg();     
    ...
    /*生成线*/
    public function Img_To_Xian($num=10,$color=NULL){
        $this->Createimg();
        if(empty($color))$color=$this->color;
        $R = hexdec(substr($color, 0, 2));
        $G = hexdec(substr($color, 2, 2));
        ...
        $color=imagecolorallocate($this->bgimg,$R,$G,$B);
        for($i=0;$i<$num;$i++){
            imageline( $this->bgimg , rand(0,$this->width) , rand(0,$this->height) , rand(30,$this->width) , rand(0,$this->height) ,$color);        
        }
    }
