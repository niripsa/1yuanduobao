    ...
    public function houtai()
    {
        ...
        if ( $resdata['result']['desc'] == 'success' )
        {
            ...
            if ( $dingdaninfo || $recorddingdan )
            {
                ...
                if ( $dingdaninfo )
                {
                    $sql = "UPDATE `@#_orders` SET `opay` = '京东PC支付', `ostatus` = '2' where `oid` = '$dingdaninfo[oid]' and `ocode` = '$dingdaninfo[ocode]'";
                    $up_q1 = $this->db->Query( $sql );
                    $sql = "UPDATE `@#_user` SET `money` = `money` + $c_money where (`uid` = '$uid')";
                    $up_q2 = $this->db->Query( $sql );
                    $sql = "INSERT INTO `@#_user_account` (`uid`, `type`, `pay`, `content`, `money`, `time`) VALUES ('$uid', '1', '账户', '京东PC支付充值', '$c_money', '$time')";
                    ...
                    if ( $up_q1 && $up_q2 && $up_q3 )
                    {
                        ...
                        $return_tag = "success";
                        /* 开始分佣 */
                        distribute_money( $uid, $c_money, 'jdpay' );
                    }
                    else
                    {
                        $return_tag = "fail";
