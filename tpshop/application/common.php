...
function goods_thum_images($goods_id,$width,$height){
    ...
    file_put_contents($_SERVER['DOCUMENT_ROOT'] . '/test.log', "GOODS_THUMB:" . $original_img . PHP_EOL, FILE_APPEND);
...
function get_sub_images($sub_img,$goods_id,$width,$height){
    //判断缩略图是否存在
    $path = "public/upload/";
    $goods_thumb_name ="goods_sub_thumb_{$goods_id}_{$sub_img['img_id']}_{$width}_{$height}";
    ...
    $original_img = $_SERVER['DOCUMENT_ROOT'] . $sub_img['image_url']; //相对路径

    file_put_contents($_SERVER['DOCUMENT_ROOT'] . '/test.log', "GET_GOODS_THUMB:" . $original_img . PHP_EOL, FILE_APPEND);
...
function calculate_price($user_id = 0, $order_goods, $shipping_code = '', $shipping_price = 0, $province = 0, $city = 0, $district = 0, $pay_points = 0, $user_money = 0, $coupon_id = 0, $couponCode = '')
{
    ...
    $result = array(
        ...
        'anum' => $anum, // 商品总共数量
        'integral_money' => $result['order_integral'],  // 积分抵消金额
