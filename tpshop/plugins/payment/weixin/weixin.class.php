...
{    
    ...
    {       
            $notify_url = SITE_URL.'/tpshop/index.php/Home/Payment/notifyUrl/pay_code/weixin'; // 接收微信支付异步通知回调地址，通知url必须为直接可访问的url，不能携带参数。
            //$notify_url = C('site_url').U('Home/Payment/notifyUrl',array('pay_code'=>'weixin')); // 接收微信支付异步通知回调地址，通知url必须为直接可访问的url，不能携带参数。
            //$notify_url = C('site_url')."/tpshop/index.php?m=Home&c=Payment&a=notifyUrl&pay_code=weixin";
            ...
            return '<img alt="模式二扫码支付" src="/tpshop/index.php?m=Home&c=Index&a=qr_code&data='.urlencode($url2).'" style="width:110px;height:110px;"/>';        
    }    
    /**
     * 服务器点对点响应操作给支付接口方调用
     * 
    ...
    function getJSAPI($order){
        ...
        $input->SetNotify_url(SITE_URL.'/tpshop/index.php/Home/Payment/notifyUrl/pay_code/weixin');
