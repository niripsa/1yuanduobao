...
function AjaxAddCart(goods_id,num,to_catr)
{
    ...
    if($("#buy_goods_form").length > 0){
        $.ajax({
            type : "POST",
            url:"/tpshop/index.php?m=Mobile&c=Cart&a=ajaxAddCart",
            ...
            success: function(data){
			   ...
			   if(to_catr == 1)  //直接购买
			   {
				   location.href = "/tpshop/index.php?m=Mobile&c=Cart&a=cart";
			   }
        ...
        $.ajax({
            ...
            success: function(data){
				   ...
				   {
					    //layer.open({content: data.msg,time: 2});
						location.href = "/tpshop/index.php?m=Mobile&c=Goods&a=goodsInfo&id="+goods_id;
				   }
				   else
				   {
					    if(data.status < 0)
